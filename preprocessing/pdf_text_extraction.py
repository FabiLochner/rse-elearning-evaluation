...
def extract_main_content(raw_text: str) -> Optional[str]:
    ...
    pattern_refs = r'^\s*(?:\d+\s*\n\s*|\d+\.?\s+)?(References|Literaturverzeichnis|Literatur|Bibliography|Bibliografie|Literature|Referenzen|Quellenverzeichnis|Quellen|Reference\s+List|Literaturverzeichnis\s+und\s+Internetquellen)\d*\s*$'
